...
**Kısa Tanıtım:**
Standard ACL ile temel erişim kontrolü (Kaynak IP tabanlı).

**Adım Adım Proje Yapımı:**
1. **Topoloji:** Bu proje Statik Yönlendirme topolojisini kullanır (ML1, R3, ML2, 4 PC).
2. **IP Yapılandırması:** Cihazlar önceden yapılandırılmıştır. PC0: 192.168.1.10, PC2: 192.168.2.10.
3. **ACL Tanımlama (R3):**
   - R3 terminaline girin: `enable`, `conf t`
   - `access-list 10 deny 192.168.1.0 0.0.0.255`
   - `access-list 10 permit any`
4. **ACL Uygulama:**
   - `interface gi0/1` (PC2 segmentine giden bacak)
   - `ip access-group 10 out`
5. **Doğrulama:**
   - `show access-lists` komutunu çalıştırın.
   - PC0 (192.168.1.10) üzerinden PC2'ye ping atmayı deneyin (başarısız olmalı).
...
**Details:** ip access-list extended WEB-FILTER, permit tcp any host 192.168.2.10 eq 80, deny ip any any

**Kısa Tanıtım:**
Extended ACL ile protokol ve port bazlı erişim kontrolü.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** Statik Yönlendirme topolojisi (ML1, R3, ML2, PC0, PC2).
2. **Extended ACL Tanımlama (R3):**
   - `enable`, `conf t`
...
4. **Test:**
   - PC0 terminalinde `wget 192.168.2.10` çalıştırın (başarılı olmalı).
   - PC0 terminalinde `ping 192.168.2.10` çalıştırın (başarısız olmalı).
   - `show access-lists` ile kural eşleşmelerini kontrol edin.
...
**Tag:** OSPF  
**Description:** Area 0 ve Area 10 ile çok alanlı OSPF yapılandırması.
...
**Adım Adım Proje Yapımı:**
1. **ML1 Yapılandırması (Area 0):**
   - `router ospf 1`
   - `network 192.168.1.0 0.0.0.255 area 0`
   - `network 10.0.0.0 0.0.0.255 area 0`
2. **R3 Yapılandırması (ABR - Area 0 & 10):**
   - `router ospf 1`
   - `network 10.0.0.0 0.0.0.255 area 0`
   - `network 20.0.0.0 0.0.0.255 area 10`
3. **ML2 Yapılandırması (Area 10):**
   - `router ospf 1`
   - `network 20.0.0.0 0.0.0.255 area 10`
...
4. **Doğrulama:**
   ...
   - `show ip route ospf`
   - PC0'dan PC2'ye (192.168.2.10) ping atın.
...
**Kısa Tanıtım:**
Gelişmiş çok alanlı OSPF ve alan özellikleri (Stub, Summary).

**Adım Adım Proje Yapımı:**
1. **Stub Area:** Bir alanı `area <id> stub` komutuyla stub olarak işaretleyin.
2. **Summarization:** ABR üzerinde `area <id> range <ip> <mask>` ile rotaları özetleyin.
3. **Test:**
   - `show ip ospf database` ile LSA tiplerini inceleyin.
   - `show ip route` ile özet rotaları kontrol edin.
...
**Kısa Tanıtım:**
L3 switch'ler arasında EIGRP (DUAL) dinamik yönlendirme.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** RIP topolojisini kullanır (ML0, ML1, PC0-3).
2. **ML0 Yapılandırması:**
   - `router eigrp 100`
   - `network 192.168.1.0 0.0.0.255`
   - `network 192.168.2.0 0.0.0.255`
   - `no auto-summary`
3. **ML1 Yapılandırması:**
   - `router eigrp 100`
   - `network 192.168.2.0 0.0.0.255`
   - `network 192.168.3.0 0.0.0.255`
   - `no auto-summary`
4. **Doğrulama:**
   - `show ip eigrp neighbors` ile komşulukları görün.
   - `show ip route eigrp` ile öğrenilen rotaları kontrol edin.
   - PC0'dan PC2'ye (192.168.3.10) ping atın.
