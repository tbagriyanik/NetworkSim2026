  ...
  return [
    {
      ...
      cmds: [
        ...
        ['Shift+Tab', isTR ? 'Ters yönde cihaz gezinimi / Görev Yöneticisi' : 'Navigate devices in reverse / Window Switcher'],
        ['Enter', isTR ? 'Seçili cihaz penceresini aç' : 'Open selected device window'],
        ['Ctrl+M', isTR ? 'Etkin pencereyi simge durumuna küçült' : 'Minimize active device window'],
        ...
        ['Ctrl+Z', isTR ? 'Geri al (Undo / Cihaz ve Hizalama geri alma)' : 'Undo (Undo move / alignment)'],
        ['Ctrl+Y', isTR ? 'İleri al (Redo / Hizalama ileri alma)' : 'Redo (Redo move / alignment)'],
        ...
        [isTR ? 'Sol Tık (Cihaz)' : 'Left Click (Device)', isTR ? 'Cihazı seç veya sürükle' : 'Select or drag device'],
        [isTR ? 'Çift Tık (Cihaz)' : 'Double Click (Device)', isTR ? 'Cihaz kontrol / yapılandırma panelini aç' : 'Open device config / control panel'],
        [isTR ? 'Sağ Tık (Cihaz / Tuval)' : 'Right Click (Device / Canvas)', isTR ? 'Bağlam menüsünü (Context Menu) aç' : 'Open context menu'],
        [isTR ? 'Fare Tekerleği (Scroll)' : 'Mouse Wheel (Scroll)', isTR ? 'Tuval üzerinde yakınlaştır / uzaklaştır (Zoom In / Out)' : 'Zoom canvas in / out'],
        [isTR ? 'Sol Tık + Sürükle (Boş Tuval)' : 'Left Click + Drag (Empty Canvas)', isTR ? 'Tuvali kaydır (Pan Canvas) veya Çoklu Alan Seçimi' : 'Pan canvas or Multi-select area'],
        [isTR ? 'Metin Seçimi (Mouse Selection)' : 'Text Selection (Mouse Selection)', isTR ? 'CMD, CLI & Konsol geçmişinden metin seçince OTOMATİK KOPYALA' : 'Auto-Copy text selection from CMD, CLI & Console outputs'],
      ]
    },
    {
      id: 'command_modes',
    ...
    },
    {
      ...
      cmds: [
        ...
        ['Alt + L', isTR ? 'Ağ Olay Günlüğü panelini aç / kapat' : 'Toggle Network Event Log panel'],
        ['Alt + F', isTR ? 'Tüm cihazları ekrana sığdır (Fit View)' : 'Zoom to fit all devices'],
        ['Shift + Tab', isTR ? 'Görev Yöneticisi (Pencere listesini aç ve cihaz pencereleri arasında geçiş yap)' : 'Open Task Switcher and switch between device windows'],
