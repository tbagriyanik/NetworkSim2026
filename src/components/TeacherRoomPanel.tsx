...
function RoomMonitor({ roomCode, onClose }: { roomCode: string; onClose: () => void }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between rounded-lg border border-slate-200/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm px-2.5 py-1.5">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onClose} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">Kapat</button>
        </div>
      </div>

      {error && <p className="text-xs text-destructive">Bağlantı hatası</p>}
      ...
      <div className="space-y-1 max-h-[calc(85vh-10rem)] overflow-y-auto">
        {students.map(s => {
          ...
          return (
            <div key={s.studentId} className="flex items-center gap-2 rounded-lg border border-slate-200/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm px-2.5 py-1.5">
              <span className="text-xs font-medium truncate min-w-0 flex-1">
                ...
                <span className="ml-1 text-[10px] text-muted-foreground/60 tabular-nums">
                  {Math.floor((now - s.joinedAt) / 60000)}dk
                </span>
              </span>
        ...
        {totalStudents === 0 && (
          <p className="py-4 text-center text-[11px] text-muted-foreground">Henüz öğrenci bağlanmadı.</p>
        )}
      </div>
    </div>
  );
...
export function TeacherRoomPanel() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-12">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-200/50 dark:border-slate-700/50 sticky top-0 bg-slate-50/40 dark:bg-slate-900/40 backdrop-blur-xl rounded-t-xl">
          <h2 className="text-base font-semibold">Öğretmen Paneli</h2>
