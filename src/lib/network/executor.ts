...
export const commandHelp: Record<string, Record<string, string[]>> = {
  ...
  'privileged': {
    ...
    ...pfx('no', ['debug']),
    'no debug': ['all'],
    ...pfx('show', ['access-lists', 'alias', 'ap', 'archive', 'arp', 'authentication', 'banner', 'boot', 'cdp', 'class-map', 'clock', 'controllers', 'debugging', 'diagnostic', 'environment', 'errdisable', 'etherchannel', 'flash', 'history', 'interface', 'interfaces', 'inventory', 'ip', 'ipv6', 'lldp', 'mac', 'memory', 'mls', 'monitor', 'ntp', 'policy-map', 'port-security', 'processes', 'redundancy', 'running-config', 'sdm', 'sessions', 'snmp', 'spanning-tree', 'ssh', 'standby', 'startup-config', 'storm-control', 'system', 'udld', 'users', 'version', 'vlan', 'vtp', 'wireless', 'wlan']),
    'show ap': ['summary'],
    'show banner': ['motd'],
    'show cdp': ['neighbors'],
    ...
    'show interface': ['trunk'],
    'show interfaces': ['status', 'trunk'],
    'show ip': ['arp', 'dhcp', 'interface', 'nat', 'ospf', 'protocols', 'route', 'source', 'ssh', 'verify'],
  ...
  'interface': {
    ...
    'no cdp': ['enable'],
    'no ip': ['access-group', 'address', 'arp', 'dhcp', 'helper-address', 'nat', 'proxy-arp'],
