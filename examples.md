...
**Details:** Rule 1: DENY ICMP | Rule 2: ALLOW ANY | PC-1 ping FAIL, wget SUCCESS.
...
**ID:** `legacy-routing`  
**Tag:** LEGACY ROUTING  
**Description:** Router connects to VLANs using two physical interfaces without trunk.  
**Details:** Router Gi0/1: VLAN 10 (192.168.0.1), Gi0/0: VLAN 20 (192.168.1.1).

**Kısa Tanıtım:**
Router'ın iki fiziksel interface kullanarak VLAN'lar arası routing yapmasını öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 Router, 1 Switch, 2 PC ekleyin.
2. **Router Konfigürasyonu:** Gi0/1 VLAN 10 (192.168.0.1/24), Gi0/0 VLAN 20 (192.168.1.1/24).
3. **Switch Konfigürasyonu:** VLAN 10/20 oluşturun, portları ilgili VLAN'lara atayın.
4. **Test:** PC-1 <-> PC-2 ping (inter-VLAN routing başarılı).

### 13. Port-Security
**ID:** `port-security`  
**Tag:** SECURITY  
**Description:** MAC address-based security restriction configured on switch port.  
**Details:** Fa0/3: max MAC 1, violation shutdown, MAC: 00-11-22-33-44-55.

**Kısa Tanıtım:**
Switch portunda MAC adres tabanlı güvenlik kısıtlaması yapılandırmayı öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 Switch ve 3 PC ekleyin.
2. **Port Güvenliği:** Fa0/3 portunda `switchport port-security maximum 1` ve `violation shutdown` yapılandırın.
3. **Test:** İzinli MAC'li PC bağlanabilir, farklı MAC'li PC bağlanamaz.

### 14. Wireless Network (WiFi)
**ID:** `wifi-intermediate`  
**Tag:** WiFi  
**Description:** Router configured as access point for wireless client connectivity.  
**Details:** SSID: HomeWiFi, Password: none (open), Router AP mode.

**Kısa Tanıtım:**
Router'ı access point olarak yapılandırıp kablosuz istemci bağlantısı sağlamayı öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 Router (AP mode) ve 1 PC (WiFi client) ekleyin.
2. **Router Konfigürasyonu:** AP moduna geçin, SSID: HomeWiFi (open) ayarlayın.
3. **PC WiFi:** PC WiFi ayarlarından HomeWiFi ağına bağlanın.
4. **Test:** PC'den router'a ping atın.

### 15. IoT WiFi Lab
**ID:** `iot-wifi-lab`  
**Tag:** IoT  
**Description:** Three IoT devices and PC connect to open WiFi network via DHCP.  
**Details:** SSID: IoT-Network, DHCP IP support, 3 IoT devices.

**Kısa Tanıtım:**
IoT cihazlarının kablosuz ağ üzerinden DHCP ile IP almasını öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 Router (AP mode), 3 IoT cihazı ve 1 PC ekleyin.
2. **Router:** SSID: IoT-Network (open) WiFi ağı oluşturun, DHCP havuzu yapılandırın.
3. **Bağlantı:** IoT cihazları ve PC WiFi üzerinden ağa bağlansın.
4. **Test:** IoT cihazlarının IP aldığını ve PC'den erişilebildiğini doğrulayın.

### 16. Greenhouse Sketch (Smart Farm)
**ID:** `greenhouse-iot-lab`  
**Tag:** ENV  
**Description:** Four environmental sensors use WPA2 WiFi for greenhouse monitoring.  
**Details:** SSID: GreenHouse-Network, Password: sera (WPA2), 4 sensors.

**Kısa Tanıtım:**
Akıllı sera senaryosunda IoT sensörlerinin WPA2 güvenli WiFi ile merkezi izleme yapmasını öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 Router (AP mode) ve 4 IoT sensör ekleyin.
2. **Router:** SSID: GreenHouse-Network, WPA2 şifre: sera WiFi ağı oluşturun.
3. **Sensörler:** Sıcaklık, nem, hareket ve ışık sensörlerini WiFi'a bağlayın.
4. **Test:** IoT panelinden sensör verilerini izleyin.

### 17. DNS + HTTP Test
**ID:** `dns-http`  
**Tag:** DNS/HTTP  
**Description:** DNS name resolution and HTTP web access are tested.  
**Details:** Test: wget 192.168.1.10, wget a10.com, nslookup a10.com.

**Kısa Tanıtım:**
DNS isim çözümleme ve HTTP web erişiminin birlikte çalışmasını test edeceksiniz.
...
**Tag:** DHCP  
**Description:** DHCP automatic IP assignment compared with manual configuration.  
**Details:** DHCP server assigns IPs to PC1 and PC2 while PC3 remains manually configured.

**Kısa Tanıtım:**
DHCP otomatik IP dağıtımı ile manuel yapılandırmayı karşılaştırmalı olarak öğreneceksiniz.
...
**ID:** `trunk-2switch`  
**Tag:** TRUNK  
**Description:** Two switches carry VLAN traffic via trunk connection.  
**Details:** SW-1/2: Fa0/1 VLAN100, Fa0/11 VLAN200, Gi0/1 trunk.

**Kısa Tanıtım:**
İki switch arasında trunk bağlantısı ile farklı VLAN'ların trafiğini taşımayı öğreneceksiniz.
...
**ID:** `l3-routing`  
**Tag:** L3 ROUTING  
**Description:** Inter-VLAN routing enabled on L3 switch for four VLANs.  
**Details:** VLAN 10: 192.168.10.1, VLAN 20: 192.168.20.1, VLAN 30: 192.168.30.1, VLAN 40: 192.168.40.1.

**Kısa Tanıtım:**
L3 switch üzerinde dört VLAN arası routing yapılandırmayı öğreneceksiniz.

**Adım Adım Proje Yapımı:**
1. **Topoloji:** 1 L3 Switch (MLS) ve 4 PC ekleyin.
2. **L3 Switch:** `ip routing` aktif edin, VLAN 10/20/30/40 oluşturun, SVI interface'lerine IP atayın.
3. **PC'ler:** Her VLAN için bir PC bağlayın, gateway SVI IP'si olsun.
4. **Test:** Farklı VLAN'lardaki PC'ler birbirine ping atabilir.
