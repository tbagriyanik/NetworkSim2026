...
- **Sınav Düzenleme Paneli Mobil Duyarlılık**: `ExamEditorPanel` genişliği `w-[450px]` → `w-full sm:w-[450px]` yapılarak mobilde tam ekrana yayılması sağlandı, `max-w-[100vw]` ile taşma engellendi.
- **Dosya Açma Filtresi Genişletildi**: Proje açma input'una `accept=".json,.exam"` eklendi; artık `.exam` dosyaları da doğrudan seçilebiliyor.
- **Sınav Bitirme Davranışı Değişti**:
  - "Sınavı Bitir" butonu süreyi donduruyor (`finishedAt` zaman damgası), paneli kapatmıyor.
  - Panel başlığı yeşile dönüyor ("Sınav Tamamlandı"), süre ve skor donuyor.
  - Otomatik görev kontrolü (auto-check) duruyor.
  - Footer'daki "Sınavı Bitir" butonu gizleniyor, kullanıcı sadece collapse edebiliyor.
  - X butonu kaldırıldı, tek collapse (küçült) butonu bırakıldı.
  - `closeExam()` ile sınav tamamen kapatılıyor (yeni proje/örnek yükleme gibi durumlarda).
- **Eklenen/Yenilenen State'ler**: `ExamProject.finishedAt`, `isExamFinished`, `closeExam()`
- **Kaydetme Onayı Eklendi**: Değişiklik varken "Yeni Proje" veya "Dosya Aç" yapılmak istendiğinde kaydetme onay dialogu gösteriliyor (`runWithSaveGuard` etkinleştirildi, `handleLoadProject`'e save guard eklendi).
...
- **37 Hazır Örnek Proje**: Temel, orta ve ileri seviye örnekler adım adım talimatlarla.
